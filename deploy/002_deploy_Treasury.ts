...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  await deploy('RomeTreasury', {
    from: deployer,
    args: [rome.address, dai, mim, frax, romefrax, calculator.address, DAO, '0'], // 1 Day timelock
    log: true,
  ...
  if (chainId == '1285') {
    await hre.run('verify:verify', {
      ...
      constructorArguments: [rome.address, dai, mim, frax, romefrax, calculator.address, DAO, '0'],
    });
  }
};
export default func;
